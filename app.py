...
def before_request():
    if request.endpoint and not request.endpoint.startswith('static'):
        ...
        if request.path == '/' or request.path == '/favicon.ico' or \
           request.path.startswith('/passengers') or request.path.startswith('/apassengers'):
            return None
        ...
        if not current_user.is_authenticated and request.endpoint not in ['login', 'static', 'logout', 'create-admin', 'index', 'passengers_home']:
...
def passenger_timetables():
    """Show timetables for local and private buses"""
    ...
    all_cities = set()
    for bus in local_buses:
        all_cities.add(bus.origin)
        all_cities.add(bus.destination)
    for bus in private_buses:
        all_cities.add(bus.origin)
        all_cities.add(bus.destination)
